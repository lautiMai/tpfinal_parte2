...
calcularDimensionesBotones() {
    textSize(18);
    this.botonAncho = max(textWidth(this.botonReiniciarTexto), textWidth(this.botonInicioTexto)) + 20; // Añadimos un margen
 ...
 dibujarBotonesFin() {
  ...
  chequearBotonesFin(mx, my) {
    if (
      ...
      objJuego = new Juego(3); // Reinicia el juego 
    } else if (
      mx > this.botonReiniciarX &&
      mx < this.botonReiniciarX + this.botonAncho &&
      ...
      pantallaInicio = new PantallaInicio(); // Regresa pantalla de inicio
      objJuego = null;
    }
  }
